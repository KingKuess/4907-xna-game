...
namespace Kuessaria
{
    class Map// this class is for creating the tiles of the background
    {
        ...
        public int[,] Read(string FileName)// this reads in the map from a file
        {
            ...
            int[,] X = new int[tempMap[0].ToCharArray().Length, tempMap.Length];//this is the int array that will be returned
            for (int i = 0; i < tempMap.Length; i++)//for the length of y
            {
                ...
                {
                    X[j, i] = (int)Char.GetNumericValue(TempX[j]);//set int[] X at position j,i to hold whatever value is at the position x(j) in row TempX(i)
                }
            }
            return X;//returns the finished array
        }

        public void Generate(int[,] map, int size)//this is what generates the map, using int size for the size of the block and map is an integer value equivalent to the block
        {
            //1 = grassblock
            //2=dirtblock
            //3 = stone block
            for (int x = 0; x < map.GetLength(0); x++)//this runs for the length of the x
            {
                ...
                {
                    int number = map[x, y];// sets number to the value of map at position X Y

                    if (number > 0)// if the number isnt 0
                        CollisionTiles.Add(new CollisionTiles(number, new Rectangle(x * size, y * size, size, size)));// it adds a new tile of type number at position x any y * size
